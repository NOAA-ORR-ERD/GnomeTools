...
def parse_args():
    ...
    parser.add_option(
        ...
        "--diff_x",
        dest="D_x",
        help=" horizontal diffusion coeff in m^2/s (%s)" % DEFAULT_D_X,
        default=DEFAULT_D_X,
        type="float",
    )
    parser.add_option(
        ...
        "--diff_z",
        dest="D_z",
        help=" vertical diffusion coeff in m^2/s (%s)" % DEFAULT_D_Z,
        default=DEFAULT_D_Z,
        type="float",
    )
    parser.add_option(
    ...
    parser.add_option(
        ...
        dest="mass_file",
        help=
        "file to calculate particle mass from (mass defaults to %s kg if no file is given)"
        % DEFAULT_MASS,
        type="string",
    )
    parser.add_option(
        ...
        dest="water_density",
        help=
        'density of the water in kg/m^3, ignored if output units is "density" (%s)'
    ...
    parser.add_option(
        ...
        dest="output_units",
        help=
        'units to use in output concentration grid: one of "density" (meaning kg/m^3), "flat" (meaning one file with kg/m^3 and a separate file with kg/m^2 and depth vertically integrated), "ratio" (meaning kg oil per kg water), "ppm" (meaning parts per million), or "ppb" (meaning parts per billion) (defaults to "%s")'
        % DEFAULT_OUTPUT_UNITS,
        default=DEFAULT_OUTPUT_UNITS,
        type="choice",
        choices=list(OUTPUT_UNITS_MAP.keys()),
    )
    parser.add_option(
        "-g",
        "--grid_file",
        dest="grid_file",
        help=
        'file with grid specification: it should specify: lon_min, lon_max, lat_min, lat_max, z_min, z_max, num_lon, num_lat, num_z (in lat-long) in python format if not specified, it will be computed to fit particles',
        default=None,
        type="string",
    )
    ...
    if len(args) < MINIMUM_ARGS_COUNT:
        parser.print_help()
        print("grid_plume takes at least %g arguments (%g given)" %
                        (MINIMUM_ARGS_COUNT, len(args)))
        sys.exit(1)
...
def load_grid(filename, proj_lat_long, proj_meters):
    ...
    grid_params = {}
    exec(compile(open(filename, "rb").read(), filename, 'exec'), grid_params)
    ...
    (x_min, y_min) = pyproj.transform(proj_lat_long, proj_meters, lon_min,
                                      lat_min)
    (x_max, y_max) = pyproj.transform(proj_lat_long, proj_meters, lon_max,
                                      lat_max)

    grid = np.zeros(
        (grid_params['num_z'], grid_params['num_lat'], grid_params['num_lon']),
        np.double)
    return (
        grid,
        x_min,
...
def main():
    ...
    x, y, z, age, times, time_units, mass, mass_units = gridplume.load_particles(
        nc.Dataset(
            particle_filename,
            "r"))  #Added age and units of mass to output variables (Zelenke).
    ...
    print("Initial masses of particles: %s kg" %
          np.unique(mass[~np.isnan(mass)]))  #(Zelenke)
    ...
    dataset = nc.Dataset(
        output_file, "w", format="NETCDF4"
    )  #Commented-out line above in favor of using version 4 NetCDF, allowing for larger file sizes and improved write performance.
    #dataset.set_fill_off() #Doesn't seem to make a difference in speed, so just leave netCDF4 defaults. (Zelenke)

    start = time.time()
    file_grid = gridplume.init_save_grid(
        dataset, grid.shape, times, time_units, meters, grid_x_min, grid_x_max,
        grid_y_min, grid_y_max, grid_z_min, grid_z_max,
    ...
    for i in range(len(seconds)):
        ...
        print("Processing time-step %g of %g (%g%%)..." %
              (i, np.max(seconds.shape), 100. * (i / np.max(seconds.shape))))
        #==============================================================================
        ...
        filter.calculate_concentration_timestep_agemassvars(
            grid,
            x[i],
            y[i],
            z[i],
