...
def load_gridplume_netcdf(path2file):
    ...
    if data.variables['concentration'].getncattr('units').lower() != 'kg/m^3':
        raise ValueError('Concentration must be in units of kg/m^3. Instead, units were %s.' %data.variables['concentration'].getncattr('units'))
    
    if data.variables['lat'].getncattr('units').lower() != 'degrees_north':
        raise ValueError('Latitude must be in units of degrees_north. Instead, units were %s.' %data.variables['lat'].getncattr('units'))
    
    if data.variables['lon'].getncattr('units').lower() != 'degrees_east':
        raise ValueError('Longitude must be in units of degrees_east. Instead, units were %s.' %data.variables['lon'].getncattr('units'))
    
    if data.variables['depth'].getncattr('units').lower() != 'meters':
        raise ValueError('Depth must be in units of meters. Instead, units were %s.' %data.variables['depth'].getncattr('units'))
    
    if data.variables['time'].getncattr('units').lower()[0:13] != 'seconds since': #Is this necessary?
        raise ValueError('Time must be in units of seconds since a date-time (e.g., Unix time). Instead, units were %s.' %data.variables['time'].getncattr('units'))
    ...
    dt = [ EpochDateTime + datetime.timedelta( seconds = np.int(time[i]) ) for i in range(len(time)) ]
...
def convert_concentration(array,units):
    ...
    if units.lower()=='ppt':
        ...
        raise ValueError('Concentration units "%s" are not supported.\nConcentration can be output in units of "ppt", "%%", "fraction", "micro-g/l", "mg/l", "ng/l", "ppb", "ppm", "pptrillion", or "lb/ft^3".' %units)
...
def max_conc_grids(path2file, units, path4output=None):
    '''
    ...
    try:
        ...
        MaxConcGrid=np.nanmax(MaxConcGrid, axis=0) #Calculate the maximum during the time-series at each grid-point, for each time.
        MaxConcGrid=np.nanmax(MaxConcGrid, axis=0) #Calculate the maximum during the time-series at each grid-point, across the depth bins.
    except ValueError:
        MaxConcGrid = np.zeros_like(Lat2D)
        for t_index in range( len(time) ):
            print "Processing time-step %g of %g (%g%%)..." %(t_index, len(time)-1, 100*(t_index/(len(time)-1)) )
            tMaxConc = data.variables['concentration'][t_index,:] #Get concentration grid for a single time-step (hopefully the results aren't so big this [z,y,x] matrix won't fit into memory... if they are, could do another loop).
            tMaxConc = np.nanmax(tMaxConc, axis=0) #Calculate the maximum during that time-step at each grid-point, across the depth bins.
            MaxConcGrid = np.fmax(MaxConcGrid, tMaxConc)
    
    
    
    #Mask-out areas with no concentration values.
    MaxConc2D=convert_concentration(MaxConcGrid,units) #Convert concentration units (i.e., from units of kg/[m^3]).
    MaxConc2D[MaxConc2D==0]=np.NaN
    
    if path4output != None:
        
        if (path4output.lower()[-3:] == ".nc") or (path4output.lower()[-4:] == ".cdf") or (path4output.lower()[-7:] == ".netcdf"):
            ...
            #Mesh coordinates.            
            lats = nc_MaxConc2D.createVariable('latitude', np.double, ('nLatitudes', 'nLongitudes'), zlib=True)
            lats.standard_name = data.variables['lat'].getncattr('standard_name')
            lats.long_name = "Latitude coordinate matrix."
            lats.units = data.variables['lat'].getncattr('units')
            lats[:] = Lat2D
            
            lons = nc_MaxConc2D.createVariable('longitude', np.double, ('nLatitudes', 'nLongitudes'), zlib=True)
            lons.standard_name = data.variables['lon'].getncattr('standard_name')
            ...
            concs = nc_MaxConc2D.createVariable('concentration', np.double, ('nLatitudes', 'nLongitudes'), zlib=True)
            ...
            setattr( nc_MaxConc2D, "Title", "GNOME GridPlume: Maximum Concentration" )
            ...
            HeaderRow = u"Latitude (\N{DEGREE SIGN}), Longitude (\N{DEGREE SIGN}), Maximum Concentration (%s)" %units
            database = np.column_stack( ( Lat2D.flatten(), Lon2D.flatten(), MaxConc2D.flatten() ) )
            ...
            np.savetxt( path4output, database, fmt='%g', delimiter=",", header=HeaderRow, comments="" )
...
def point_conc(path2file, units, PointLat, PointLon, path4output=None):
    ...
    zi = np.nanargmax(ConcAtPt, axis=1) #Depth indices of grid-point's maximum concentrations at each time-step.
    DepthOfMaxConcAtPt = depth[zi] #Depths of maximum concentrations at grid-point.
    
    
    MaxConcAtPt = convert_concentration(MaxConcAtPt, units) #Convert concentration units (i.e., from units of kg/[m^3]).
    ...
    if path4output != None:
        ...
        if (path4output.lower()[-3:] == ".nc") or (path4output.lower()[-4:] == ".cdf") or (path4output.lower()[-7:] == ".netcdf"):
            ...
            times = nc_PointConc2D.createVariable('time', np.double, ('nTimes',), zlib=True)
            times.standard_name = data.variables['time'].getncattr('standard_name')
            times.long_name = data.variables['time'].getncattr('long_name')
            times.units = data.variables['time'].getncattr('units')
            times[:] = time
            
            lats = nc_PointConc2D.createVariable('latitude', np.double, ('One',), zlib=True)
            lats.standard_name = data.variables['lat'].getncattr('standard_name')
            lats.long_name = data.variables['lat'].getncattr('long_name')
            lats.units = data.variables['lat'].getncattr('units')
            lats[:] = ptLat
            
            lons = nc_PointConc2D.createVariable('longitude', np.double, ('One',), zlib=True)
            lons.standard_name = data.variables['lon'].getncattr('standard_name')
            lons.long_name = data.variables['lon'].getncattr('long_name')
            lons.units = data.variables['lon'].getncattr('units')
            lons[:] = ptLon
            
            depths = nc_PointConc2D.createVariable('depth', np.double, ('nTimes',), zlib=True)
            depths.standard_name = data.variables['depth'].getncattr('standard_name')
            ...
            concs = nc_PointConc2D.createVariable('concentration', np.double, ('nTimes',), zlib=True)
            concs.short_name = "mass_concentration_of_pollutant_in_sea_water" #Note:  not a standard name.
            ...
            #Set file attributes.
            #setattr( nc_MaxConc2D, "Conventions", "CF-1.6" )
            setattr( nc_PointConc2D, "Title", "GNOME GridPlume: Maximum Concentrations at Coordinate" )
            ...
            HeaderRow = u"Time, Maximum Concentration (%s), Depth of Maximum (m), Latitude for All Values (\N{DEGREE SIGN}), Longitude for All Values (\N{DEGREE SIGN})\n%s,%g,%g,%g,%g" %(units, dt[0], MaxConcAtPt[0], DepthOfMaxConcAtPt[0], ptLat, ptLon)
            database = np.column_stack( ( dt[1:], MaxConcAtPt[1:], DepthOfMaxConcAtPt[1:] ) )
            np.savetxt( path4output, database, fmt=['%s', '%g', '%g'], delimiter=",", header=HeaderRow, comments="" )
...
def max_conc_in_plume(path2file, units, path4output=None):
    '''
    Calculate the time-series of the highest concentration modeled at any depth
    ...
    try:
        ...
        MaxConcSeries = np.nanmax(MaxConcSeries, axis=1) #Calculate the maximum across the depth dimension.
        MaxConcSeries = np.nanmax(MaxConcSeries, axis=1) #Calculate the maximum across the latitude dimension.
        MaxConcSeries = np.nanmax(MaxConcSeries, axis=1) #Calculate the maximum across the longitude dimension.
    except ValueError:
    ...
    MaxConcSeries[MaxConcSeries==0] = np.NaN #Now just a time-series of maximums in the time dimension remains (less any times where the plume dissipated entirely.)
    MaxConcSeries = convert_concentration(MaxConcSeries, units) #Convert concentration units (i.e., from units of kg/[m^3]).
    ...
    if path4output != None:
        ...
        if (path4output.lower()[-3:] == ".nc") or (path4output.lower()[-4:] == ".cdf") or (path4output.lower()[-7:] == ".netcdf"):
            ...
            #Set file attributes.
            #setattr( nc_MaxConc2D, "Conventions", "CF-1.6" )
            setattr( nc_PointConc2D, "Title", "GNOME GridPlume: Maximum Concentrations Modeled" )
            setattr( nc_PointConc2D, "Institution", "USA/DOC/NOAA/NOS/ERD/TSSB" )
            setattr( nc_PointConc2D, "References", "brian.zelenke@noaa.gov" )
            ...
            HeaderRow = u"Time, Maximum Concentration (%s)" %units
            database = np.column_stack( ( dt, MaxConcSeries ) )
            np.savetxt( path4output, database, fmt=['%s', '%g'], delimiter=",", header=HeaderRow, comments="" )
    
    data.close()
    
    return ( MaxConcSeries,
             time,
             )
...
def make_3ColorFig(Lon2D, Lat2D, masked_array, units, levels, FontPtSize=14, isGoogleEarthKMZ=False):
    '''
    ...
    if MaxConc2Plot < maxConc and MinConc2Plot > minConc:
        cssurf = csax.contourf(Lon2D, Lat2D, masked_array, levels, colors=['blue','black','green'], extend="both")
    elif MaxConc2Plot < maxConc and MinConc2Plot <= minConc:
        cssurf = csax.contourf(Lon2D, Lat2D, masked_array, levels, colors=['blue','black','green'], extend="max")
    elif MaxConc2Plot >= maxConc and MinConc2Plot > minConc:
        cssurf = csax.contourf(Lon2D, Lat2D, masked_array, levels, colors=['blue','black','green'], extend="min")
    else:
        cssurf = csax.contourf(Lon2D, Lat2D, masked_array, levels, colors=['blue','black','green'], extend="neither")
    ...
    if isGoogleEarthKMZ:
        ...
        cscbar.set_label('Concentration (%s)' %units, fontsize=FontPtSize, fontweight='bold')
        cscbar.ax.tick_params(labelsize=FontPtSize) #Do you want to add a tick size parameter?
        ...
        csax.set_xlabel('Longitude ($\degree$)', fontsize=FontPtSize, fontweight='bold')
        csax.set_ylabel('Latitude ($\degree$)', fontsize=FontPtSize, fontweight='bold')
        csax.set_title('Highest Concentration Reached Throughout\nAll Depths Anytime During Model Run', fontsize=FontPtSize+2, fontweight='bold')
...
def make_pColorFig(Lon2D, Lat2D, masked_array, units, MinConc2Plot=None, MaxConc2Plot=None, FontPtSize=14, isGoogleEarthKMZ=False):
    '''
    ...
    if isGoogleEarthKMZ:
        ...
        pcsurf = pcax.pcolor(Lon2D, Lat2D, masked_array, shading='interp', edgecolors='none')
        ...
        pccbar.set_label('Concentration (%s)' %units, fontsize=FontPtSize, fontweight='bold')
        pccbar.ax.tick_params(labelsize=FontPtSize) #Do you want to add a tick size parameter?
        cbarfig.delaxes( cbarfig.gca() )
        #cbarfig.savefig(path4cbar, transparent=False, bbox_inches='tight')
        ...
        pcsurf = pcax.pcolor(Lon2D, Lat2D, masked_array, shading='interp', edgecolors='none')
        pcsurf.set_clim(MinConc2Plot, MaxConc2Plot) #Values outside range are saturated.
        pcax.ticklabel_format(style='plain', useOffset=False, axis='both')
        pcax.tick_params( labelsize=FontPtSize )
        pcax.set_xlabel('Longitude ($\degree$)', fontsize=FontPtSize, fontweight='bold')
        pcax.set_ylabel('Latitude ($\degree$)', fontsize=FontPtSize, fontweight='bold')
        pcax.set_title('Highest Concentration Reached Throughout\nAll Depths Anytime During Model Run', fontsize=FontPtSize+2, fontweight='bold')
...
def write_KMZ(path4KMZ, ColorBarFig, OverlayFig, WestLonLim, EastLonLim, SouthLatLim, NorthLatLim):
    '''
    ...
    fKML.write( '<kml xmlns="http://www.opengis.net/kml/2.2" xmlns:gx="http://www.google.com/kml/ext/2.2" xmlns:kml="http://www.opengis.net/kml/2.2" xmlns:atom="http://www.w3.org/2005/Atom">\n' )
    
    fKML.write( '<Folder>\n' )
    fKML.write( '\t<name>GridPlume</name>\n' )
    fKML.write( '\t<description>Highest Concentration Reached Throughout All Depths Anytime During Model Run.\n' )
    ...
    fKML.write( '\t\t</Icon>\n' )
    fKML.write( '\t\t<overlayXY x="0" y="1" xunits="fraction" yunits="fraction"/>\n' )
    fKML.write( '\t\t<screenXY x="0" y="1" xunits="fraction" yunits="fraction"/>\n' )
    fKML.write( '\t\t<rotationXY x="0.5" y="0.5" xunits="fraction" yunits="fraction"/>\n' )
...
def plot_max_conc_everywhere(path2file, units, path4image=None, MinConc2Plot=None, MaxConc2Plot=None, Just3Colors=False, LowConcUpperLim=None, HighConcLowerLim=None, FontPtSize=14):
    ...
    Lon2D, Lat2D, MaxConcGrid = max_conc_grids(path2file, units)
    ...
    MinConc2Plot, MaxConc2Plot = np.sort( [MinConc2Plot, MaxConc2Plot] ) #Quality control.
    ...
    if Just3Colors:
        ...
        levels = np.linspace( np.nanmin(MinConc2Plot), np.nanmax(MaxConc2Plot), 4 )
        if LowConcUpperLim == None:
            LowConcUpperLim = levels[1]
        if HighConcLowerLim == None:
            HighConcLowerLim = levels[2]
        LowConcUpperLim, HighConcLowerLim = np.sort( [LowConcUpperLim, HighConcLowerLim] )
        ...
        fig, minX, maxX, minY, maxY, cbfig = make_3ColorFig( Lon2D, Lat2D, masked_array, units, levels, FontPtSize, isGoogleEarthKMZ )
        
    else:
        fig, minX, maxX, minY, maxY, cbfig = make_pColorFig( Lon2D, Lat2D, masked_array, units, MinConc2Plot, MaxConc2Plot, FontPtSize, isGoogleEarthKMZ )
    ...
    plt.show(fig) #Show the figure on the screen.  If a KMZ was specified, this will just be the tile sans labels.
...
def plot_conc_at_point(path2file, units, PointLat, PointLon, path4image=None, FontPtSize=14):
    ...
    MaxConcAtPt, time, pLat, pLon, _ = point_conc(path2file, units, PointLat, PointLon)
    
    tHours = time / (60*60) #Convert time from seconds elapsed (Unix time) to hours elapsed.
    ...
    ax.set_xlabel('Hours from Release Time', fontsize=FontPtSize, fontweight='bold')
    ax.set_ylabel('Concentration (%s)' %units, fontsize=FontPtSize, fontweight='bold')
    ax.set_title(r'Time-series of Highest Concentration within Water Column at %g$^{\circ}$, %g$^{\circ}$' %(pLat,pLon), fontsize=FontPtSize+2, fontweight='bold')
...
def plot_max_conc_in_plume(path2file, units, path4image=None, FontPtSize=14):
    ...
    MaxConcSeries, time = max_conc_in_plume(path2file, units)
    ...
    ax.set_xlabel('Hours from Release Time', fontsize=FontPtSize, fontweight='bold')
    ax.set_ylabel('Concentration (%s)' %units, fontsize=FontPtSize, fontweight='bold')
    ax.set_title(r'Time-series of Highest Concentration within Water Column Throughout Model Domain', fontsize=FontPtSize+2, fontweight='bold')
